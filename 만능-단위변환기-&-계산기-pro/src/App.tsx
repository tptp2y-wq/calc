...
export default function App() {
  ...
  const handleShare = (platform: 'naver' | 'facebook' | 'twitter' | 'band' | 'kakaostory' | 'instagram' | 'copy' | 'webshare') => {
    const shareTitle = "만능 단위변환기 & 계산기 Pro | 일반 사칙연산, 퍼센트, 연봉 실수령액, 평수 환산";
    const shareDescription = "길이·무게·넓이·부피·온도·데이터 단위변환, 평수 환산, 퍼센트 계산, 연봉 실수령액 계산, 공학용 수식 및 방정식 풀이를 지원하는 만능 계산기";
    ...
    switch (platform) {
      ...
      case 'twitter':
        ...
        break;
      case 'band':
        url = `https://band.us/plugin/share?body=${encodeURIComponent(shareTitle + '\n' + shareUrl)}`;
        break;
      case 'kakaostory':
        url = `https://story.kakao.com/s/share?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(shareTitle)}`;
        break;
      case 'instagram':
        navigator.clipboard.writeText(shareUrl).then(() => {
          setShareLinkCopied(true);
          setTimeout(() => setShareLinkCopied(false), 2000);
          alert("만능 단위변환기 주소가 클립보드에 복사되었습니다!\n\n인스타그램 앱에서 '스토리 링크 스티커'로 사용하시거나 프로필 링크에 등록하여 공유해 보세요.");
          window.open('https://instagram.com', '_blank', 'noreferrer');
        });
        break;
    }

    if (url) {
      window.open(url, '_blank', 'width=600,height=500,noreferrer');
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col justify-between font-sans transition-colors duration-300">
      ...
      <header className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/80 transition-colors">
        <div className="max-w-7xl mx-auto px-4 py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          ...
          <div className="flex items-center gap-3 flex-1 md:max-w-lg md:justify-end">
            ...
            {/* SNS Share Button & Dropdown */}
            <div className="relative shrink-0" id="header-sns-share-container">
              <button
                onClick={() => setShowShareMenu(!showShareMenu)}
                className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-sky-50 to-blue-50 hover:from-sky-100 hover:to-blue-100 dark:from-sky-950/40 dark:to-blue-950/40 dark:hover:from-sky-900/40 dark:hover:to-blue-900/40 text-xs font-bold rounded-xl text-sky-700 dark:text-sky-300 border border-sky-100/60 dark:border-sky-900/30 transition-all cursor-pointer shadow-sm"
                title="SNS 공유하기"
                ...
                {showShareMenu && (
                  <>
                    ...
                    <motion.div
                      ...
                      transition={{ duration: 0.15 }}
                      className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-xl z-50 p-2 space-y-1"
                    >
                      <div className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider px-3 py-1.5 border-b border-slate-50 dark:border-slate-800">
                        SNS 공유 및 주소 복사
                      </div>
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-between px-3.5 py-1.5 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 text-slate-700 dark:text-slate-300 rounded-xl transition-all cursor-pointer text-xs"
                      >
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-md bg-emerald-500 flex items-center justify-center text-white text-[10px] font-extrabold leading-none">N</span>
                          <span className="font-semibold">네이버 블로그</span>
                        </div>
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-md bg-[#21C45E] flex items-center justify-center text-white text-[10px] font-extrabold leading-none">B</span>
                          <span className="font-semibold">네이버 밴드</span>
                        </div>
                        <ExternalLink size={11} className="text-slate-400" />
                      </button>
                      ...
                      {/* KakaoStory */}
                      <button
                        onClick={() => handleShare('kakaostory')}
                        className="w-full flex items-center justify-between px-3.5 py-1.5 hover:bg-amber-50 dark:hover:bg-amber-950/20 text-slate-700 dark:text-slate-300 rounded-xl transition-all cursor-pointer text-xs"
                      >
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-md bg-yellow-400 flex items-center justify-center text-amber-950 text-[10px] font-extrabold leading-none">K</span>
                          <span className="font-semibold">카카오스토리</span>
                        </div>
                        <ExternalLink size={11} className="text-slate-400" />
                      </button>
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-between px-3.5 py-1.5 hover:bg-blue-50 dark:hover:bg-blue-950/20 text-slate-700 dark:text-slate-300 rounded-xl transition-all cursor-pointer text-xs"
                      >
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-md bg-[#1877F2] flex items-center justify-center text-white text-[10px] font-extrabold leading-none">F</span>
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-between px-3.5 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl transition-all cursor-pointer text-xs"
                      >
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-md bg-black dark:bg-slate-800 flex items-center justify-center text-white text-[10px] font-extrabold leading-none">X</span>
                          <span className="font-semibold">트위터 (X)</span>
                        </div>
                        <ExternalLink size={11} className="text-slate-400" />
                      </button>
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-between px-3.5 py-1.5 hover:bg-pink-50 dark:hover:bg-pink-950/20 text-slate-700 dark:text-slate-300 rounded-xl transition-all cursor-pointer text-xs"
                      >
                        <div className="flex items-center gap-2">
                          <span className="w-5 h-5 rounded-md bg-gradient-to-tr from-[#F58529] via-[#DD2A7B] to-[#8134AF] flex items-center justify-center text-white text-[10px] font-extrabold leading-none">I</span>
                      ...
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3.5 py-2 rounded-xl transition-all cursor-pointer text-xs font-bold ${
                          shareLinkCopied 
                      ...
                      >
                        <div className="flex items-center gap-2">
                          {shareLinkCopied ? <Check size={13} /> : <Copy size={13} />}
                          <span>{shareLinkCopied ? '주소 복사 완료!' : '페이지 주소 복사'}</span>
                        </div>
                        {!shareLinkCopied && <span className="text-[9px] text-slate-400 dark:text-slate-500">Ctrl+C</span>}
                      </button>
                    </motion.div>
                  </>
                )}
      ...
      {/* AADS AD UNIT 2448351 */}
      <div className="w-full mt-4 px-4 animate-fade-in" id="aads-ad-container">
        <div className="max-w-7xl mx-auto bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-850 p-3 rounded-2xl shadow-sm">
          <div id="frame" style={{ width: '100%', margin: 'auto', position: 'relative', zIndex: 99998 }}>
            <iframe 
              data-aa="2448351" 
              src="https://ad.a-ads.com/2448351?size=Adaptive"
